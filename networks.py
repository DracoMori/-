...
User_dict_path = 'D:/competition/泰迪杯_C/data_raw'
jieba.load_userdict(os.path.join(User_dict_path, "User_dict.txt"))

print(jieba.lcut('性价比不高'))
            ...
            for m in range(len(words)):
                ...
                if word in hp.posdict: 
                    ...
                    if word in ['好','真','实在'] and words[min(i+1,len(words)-1)] in hp.pos_neg_dict  and words[min(i+1,len(words)-1)] != word:
                        ...
                        for w in words[a:i]: # 扫描情感词前的程度词
                            if w in hp.mostdict:
                                ...
                                poscount *= -0.5 
                            elif w in hp.inversedict or '不' in w:  # 带不的都认为是否定词
                                c+= 1
                            else:
                                poscount *= 1
                        # print('发现%s前存在%d重否定' % (word, c))
                        if tool.is_odd(c) == 'odd': # 扫描情感词前的否定词数
                            ...
                            poscount = 0
                        a = i+1
                elif word in hp.negdict  or ('不' in word[0] and len(word)>1 and word not in hp.posdict): # 消极情感的分析，与上面一致              
                    # print('发现消极词：%s'%word)
                    if word in ['好','真','实在'] and words[min(i+1,len(words)-1)] in hp.pos_neg_dict and words[min(i+1,len(words)-1)] != word:
                        continue
                    else:
                        ...
                        for w in words[a:i]:                         
                            if w in hp.mostdict:
                                ...
                                negcount *= -0.5
                            elif w in hp.inversedict or '不' in w:
                                d += 1
                            else:
                                negcount *= 1
                    # print('发现%s前存在%d重否定' % (word, d))
                    if tool.is_odd(d) == 'odd':
                        ...
                        negcount = 0
                    a = i + 1                       
                elif word == '不' and  (m+1) < len(words) and words[m+1] not in hp.negdict and words[m+1] not in hp.posdict:
                    # print('发现消极词：%s'%(word+words[m+1]))
                    if word in ['好','真','实在'] and words[min(i+1,len(words)-1)] in hp.pos_neg_dict and words[min(i+1,len(words)-1)] != word:
                        continue
                    else:
                        negcount += 1
    ...
    def sentiment_score(self,s):
        ...
        if senti_score_list != []:
            ...
            if pos_score >=0 and neg_score<=0:
                pos_score = pos_score + abs(neg_score)
                neg_score = 0
            elif pos_score < 0 and neg_score>=0:
                neg_score = neg_score + abs(pos_score)
                pos_score = 0
            elif pos_score < 0 and neg_score < 0:
                pos_score = abs(neg_score)  
                neg_score = abs(pos_score)
        else:
